...
//			| STATE_NZ_CTXT_R1 | STATE_NZ_CTXT_R2;
	...
	public DataBlk getCodeBlock(final int c, final int m, final int n, final SubbandSyn sb, DataBlk cblk)
	{
		...
		{
			isterm = 0 != (options & OPT_TERM_PASS)
					|| (0 != (options & OPT_BYPASS) && (31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - this.srcblk.skipMSBP) >= curbp);
		...
		if (!error || !this.doer)
		{
			while (0 <= curbp && 0 < npasses)
			{
				...
				if (0 != (options & OPT_BYPASS) && (curbp < 31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - this.srcblk.skipMSBP))
				{
				...
				if (0 != (options & OPT_TERM_PASS)
						|| (0 != (options & OPT_BYPASS) && (curbp < 31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - this.srcblk.skipMSBP)))
				{
					// Here starts a new MQ segment
					++tsidx;
					this.mq.nextSegment(null, -1, this.srcblk.tsLengths[tsidx]);
	...
	private boolean sigProgPass(final DataBlk cblk, final MQDecoder mq, final int bp, final int[] state, final int[] zc_lut, final boolean isterm)
	{
		...
		causal = 0 != (options & OPT_VERT_STR_CAUSAL);
		...
		for (s = nstripes - 1; 0 <= s; s--, sk += kstep, sj += jstep)
		{
			...
			for (; sk < stopsk; sk++, sj++)
			{
				...
				top_half:
				{
					...
					if (0 == csj && 0 == state[j + sscanw] && STRIPE_HEIGHT == sheight)
					{
