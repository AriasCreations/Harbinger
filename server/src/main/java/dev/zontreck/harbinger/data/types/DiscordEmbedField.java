...
public class DiscordEmbedField implements IJsonSerializable
{
	...
	public JSONObject serialize() throws DiscordEmbedLimitsException {
		...
		if(!name.isEmpty())
		{
			if(name.length()>256)throw new DiscordEmbedLimitsException("Field name must be less than 256 characters");
			obj.put("name", name);
		}

		if(!value.isEmpty())
		{
			if(value.length()>4096)throw new DiscordEmbedLimitsException("Field value must be less than 4096 characters");
			obj.put("value", value);
		}
