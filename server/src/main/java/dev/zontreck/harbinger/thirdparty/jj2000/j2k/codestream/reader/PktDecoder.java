...
public class PktDecoder implements StdEntropyCoderOptions {
	 ...
	 */
	public PktDecoder (
			final DecoderSpecs decSpec , final HeaderDecoder hd , final RandomAccessIO ehs , final BitstreamReaderAgent src ,
	 ...
	 * @param mdl     The maximum number of decomposition level in each component of
	 *                this tile
	 ...
	 */
	public CBlkInfo[][][][][] restart (
			final int nc , final int[] mdl , final int nl , CBlkInfo[][][][][] cbI , final boolean pph ,
			...
			for ( int j = jstart ; j <= jend ; j++ , nPrec++ ) { // Horizontal precincts
				...
				if ( 0 == r ) { // LL subband
					...
					if ( 0 >= s1x - s0x || 0 >= s1y - s0y ) {
						...
						this.ttIncl[ c ][ 0 ][ nPrec ][ 0 ] = new TagTreeDecoder ( 0 , 0 );
						this.ttMaxBP[ c ][ 0 ][ nPrec ][ 0 ] = new TagTreeDecoder ( 0 , 0 );
					}
					else {
			...
			for ( int m = 0 ; m < mend ; m++ ) { // Vertical code-blocks
				...
				for ( int n = 0 ; n < nend ; n++ ) { // Horizontal code-blocks
					...
					try {
						...
						if ( 1 == bin.readBit ( ) ) {// if bit is 1
							totnewtp++;

							// if next bit is 0 do nothing
							if ( 1 == bin.readBit ( ) ) {// if is 1
								totnewtp++;
...
//						sumtotnewtp += totnewtp;
						...
						if ( 0 != ( options & StdEntropyCoderOptions.OPT_TERM_PASS ) ) {
							...
							if ( StdEntropyCoderOptions.FIRST_BYPASS_PASS_IDX >= ccb.ctp ) {
								nSeg = 1;
							}
							else {
						...
						if ( 1 == nSeg ) { // Only one segment in packet
