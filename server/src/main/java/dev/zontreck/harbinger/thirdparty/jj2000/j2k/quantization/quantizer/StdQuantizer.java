 ...
 */
package dev.zontreck.harbinger.thirdparty.jj2000.j2k.quantization.quantizer;
...
public class StdQuantizer extends Quantizer {
	 ...
	 */
	public static final int QSTEP_MAX_MANTISSA = ( 1 << StdQuantizer.QSTEP_MANTISSA_BITS ) - 1;

	/**
	 * The maximum value of the exponent for the quantization steps
	 */
	public static final int QSTEP_MAX_EXPONENT = ( 1 << StdQuantizer.QSTEP_EXPONENT_BITS ) - 1;
	 ...
	 */
	private final GuardBitsSpec gbs;
	 ...
	 */
	public StdQuantizer ( final CBlkWTDataSrc src , final EncoderSpecs encSpec ) {
		super ( src );
	 ...
	 */
	public static int convertToExpMantissa ( final float step ) {
		final int exp;

		exp = ( int ) Math.ceil ( - Math.log ( step ) / StdQuantizer.log2 );
		if ( QSTEP_MAX_EXPONENT < exp ) {
			// If step size is too small for exponent representation, use the
			// minimum, which is exponent QSTEP_MAX_EXPONENT and mantissa 0.
			return ( StdQuantizer.QSTEP_MAX_EXPONENT << StdQuantizer.QSTEP_MANTISSA_BITS );
		}
		...
		return ( exp << StdQuantizer.QSTEP_MANTISSA_BITS ) | ( ( int ) ( ( - step * ( - 1 << exp ) - 1.0f ) * ( 1 << StdQuantizer.QSTEP_MANTISSA_BITS ) + 0.5f ) );
	}
	 ...
	 * dynamic range of 1.
	 */
	private static float convertFromExpMantissa ( final int ems ) {
		// NOTE: this formula does not support more than 5 bits for the
		// exponent, otherwise (-1<<exp) might overflow (the - is used to be
		// able to represent 2**31)
		return ( - 1.0f - ( ( float ) ( ems & StdQuantizer.QSTEP_MAX_MANTISSA ) ) / ( 1 << StdQuantizer.QSTEP_MANTISSA_BITS ) )
				/ ( - 1 << ( ( ems >> StdQuantizer.QSTEP_MANTISSA_BITS ) & StdQuantizer.QSTEP_MAX_EXPONENT ) );
	}
	 ...
	 */
	@Override
	public int getNumGuardBits ( final int t , final int c ) {
		return ( ( Integer ) this.gbs.getTileCompVal ( t , c ) ).intValue ( );
	}
	 ...
	 */
	@Override
	public final CBlkWTData getNextInternCodeBlock ( final int c , CBlkWTData cblk ) {
		...
		final boolean intq; // flag for quantizig ints
		final SubbandAn sb;
		float stepUDR; // The quantization step size (for a dynamic
		// range of 1, or unit)
		final int g = ( ( Integer ) this.gbs.getTileCompVal ( this.tIdx , c ) ).intValue ( );

		// Are we quantizing ints or floats?
		intq = ( DataBlk.TYPE_INT == src.getDataType ( tIdx , c ) );
		...
		if ( intq ) { // Source data is int
			...
			infblk = ( CBlkWTDataFloat ) this.src.getNextInternCodeBlock ( c , infblk );
			if ( null == infblk ) {
			...
			// quantized coeffs in
			outarr = ( int[] ) cblk.getData ( );
			if ( null == outarr || outarr.length < infblk.w * infblk.h ) {
		...
		if ( this.isReversible ( this.tIdx , c ) ) { // Reversible only for int data
			...
			for ( j = w * h - 1; 0 <= j ; j-- ) {
				...
				outarr[ j ] = ( ( 0 > tmp ) ? ( 1 << 31 ) | ( - tmp ) : tmp );
			}
		}
		else { // Non-reversible, use step size
			final float baseStep = ( ( Float ) this.qsss.getTileCompVal ( this.tIdx , c ) ).floatValue ( );

			// Calculate magnitude bits and quantization step size
			if ( this.isDerived ( this.tIdx , c ) ) {
				cblk.magbits = g - 1 + sb.level - ( int ) Math.floor ( Math.log ( baseStep ) / StdQuantizer.log2 );
				stepUDR = baseStep / ( 1 << sb.level );
			}
			else {
				cblk.magbits = g - 1 - ( int ) Math.floor ( Math.log ( baseStep / ( sb.l2Norm * ( 1 << sb.anGainExp ) ) ) / StdQuantizer.log2 );
			...
			invstep = 1.0f / ( ( 1L << ( this.src.getNomRangeBits ( c ) + sb.anGainExp ) ) * stepUDR );
			// Normalize to magnitude bits (output fractional point)
			invstep *= ( 1 << ( shiftBits - this.src.getFixedPoint ( c ) ) );

			// Update convertFactor and stepSize fields
			cblk.convertFactor = invstep;
			cblk.stepSize = ( ( 1L << ( this.src.getNomRangeBits ( c ) + sb.anGainExp ) ) * stepUDR );
			...
			if ( intq ) { // Quantizing int data
				...
				for ( j = w * h - 1 , k = infblk.offset + ( h - 1 ) * infblk.scanw + w - 1 , jmin = w * ( h - 1 ); 0 <= j ; jmin -= w ) {
					for ( ; j >= jmin ; k-- , j-- ) {
	 ...
	 */
	@Override
	protected void calcSbParams ( final SubbandAn sb , final int c ) {
		final float baseStep;

		if ( 0.0f < sb.stepWMSE ) // parameters already calculated
			return;
		if ( ! sb.isNode ) {
			if ( this.isReversible ( this.tIdx , c ) ) {
				sb.stepWMSE = ( float ) Math.pow ( 2 , - ( this.src.getNomRangeBits ( c ) << 1 ) ) * sb.l2Norm * sb.l2Norm;
			}
			else {
				baseStep = ( ( Float ) this.qsss.getTileCompVal ( this.tIdx , c ) ).floatValue ( );
				if ( this.isDerived ( this.tIdx , c ) ) {
			...
			sb.stepWMSE = 1.0f; // Signal that we already calculated this branch
		}
	}
	 ...
	 */
	@Override
	public int getMaxMagBits ( final int c ) {
		final Subband sb = this.getAnSubbandTree ( this.tIdx , c );
		if ( this.isReversible ( this.tIdx , c ) ) {
	 ...
	 */
	private int getMaxMagBitsRev ( final Subband sb , final int c ) {
		int tmp, max = 0;
		final int g = ( ( Integer ) this.gbs.getTileCompVal ( this.tIdx , c ) ).intValue ( );
		...
		tmp = this.getMaxMagBitsRev ( sb.getLH ( ) , c );
		if ( tmp > max )
			max = tmp;
		tmp = this.getMaxMagBitsRev ( sb.getHL ( ) , c );
		if ( tmp > max )
			max = tmp;
		tmp = this.getMaxMagBitsRev ( sb.getHH ( ) , c );
		if ( tmp > max )
			max = tmp;
	 ...
	 * @return The highest number of magnitude bit-planes
	 */
	private int getMaxMagBitsDerived ( final Subband sb , final int t , final int c ) {
		int tmp, max = 0;
		final int g = ( ( Integer ) this.gbs.getTileCompVal ( t , c ) ).intValue ( );

		if ( ! sb.isNode ) {
			final float baseStep = ( ( Float ) this.qsss.getTileCompVal ( t , c ) ).floatValue ( );
			return g - 1 + sb.level - ( int ) Math.floor ( Math.log ( baseStep ) / StdQuantizer.log2 );
		}

		max = this.getMaxMagBitsDerived ( sb.getLL ( ) , t , c );
		tmp = this.getMaxMagBitsDerived ( sb.getLH ( ) , t , c );
		if ( tmp > max )
			max = tmp;
		tmp = this.getMaxMagBitsDerived ( sb.getHL ( ) , t , c );
		if ( tmp > max )
			max = tmp;
		tmp = this.getMaxMagBitsDerived ( sb.getHH ( ) , t , c );
		if ( tmp > max )
			max = tmp;
	 ...
	 * @return The highest number of magnitude bit-planes
	 */
	private int getMaxMagBitsExpounded ( final Subband sb , final int t , final int c ) {
		int tmp, max = 0;
		final int g = ( ( Integer ) this.gbs.getTileCompVal ( t , c ) ).intValue ( );
		...
		if ( ! sb.isNode ) {
			final float baseStep = ( ( Float ) this.qsss.getTileCompVal ( t , c ) ).floatValue ( );
			return g - 1
					- ( int ) Math.floor ( Math.log ( baseStep / ( ( ( SubbandAn ) sb ).l2Norm * ( 1 << sb.anGainExp ) ) ) / StdQuantizer.log2 );
		}

		max = this.getMaxMagBitsExpounded ( sb.getLL ( ) , t , c );
		tmp = this.getMaxMagBitsExpounded ( sb.getLH ( ) , t , c );
		if ( tmp > max )
			max = tmp;
		tmp = this.getMaxMagBitsExpounded ( sb.getHL ( ) , t , c );
		if ( tmp > max )
			max = tmp;
		tmp = this.getMaxMagBitsExpounded ( sb.getHH ( ) , t , c );
		if ( tmp > max )
			max = tmp;
