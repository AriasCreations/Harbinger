...
public class HeaderInfo implements Markers, ProgressionType, FilterTypes, Cloneable {
	...
	public class SIZ implements Cloneable {
		...
		public int getCompImgWidth ( final int c ) {
			if ( null == compWidth ) {
				...
				for ( int cc = 0 ; cc < this.csiz ; cc++ ) {
					this.compWidth[ cc ] = ( int ) (
							Math.ceil ( ( this.xsiz ) / ( double ) this.xrsiz[ cc ] ) - Math.ceil ( this.x0siz
		...
		public int getCompImgHeight ( final int c ) {
			if ( null == compHeight ) {
				...
				for ( int cc = 0 ; cc < this.csiz ; cc++ ) {
					this.compHeight[ cc ] = ( int ) (
							Math.ceil ( ( this.ysiz ) / ( double ) this.yrsiz[ cc ] ) - Math.ceil ( this.y0siz
		...
		public boolean isOrigSigned ( final int c ) {
			if ( null == origSigned ) {
				...
				for ( int cc = 0 ; cc < this.csiz ; cc++ ) {
					this.origSigned[ cc ] = ( 1 == ( ssiz[ cc ] >>> Markers.SSIZ_DEPTH_BITS ) );
				}
			}
	...
	public class COD implements Cloneable {
		...
		public String toString ( ) {
			...
			if ( 0 == scod ) {
				...
				if ( 0 != ( scod & Markers.SCOX_PRECINCT_PARTITION ) )
					str += "Precints ";
				if ( 0 != ( scod & Markers.SCOX_USE_SOP ) )
					str += "SOP ";
				if ( 0 != ( scod & Markers.SCOX_USE_EPH ) )
					str += "EPH ";
				final int cb0x = ( 0 != ( scod & Markers.SCOX_HOR_CB_PART ) ) ? 1 : 0;
				final int cb0y = ( 0 != ( scod & Markers.SCOX_VER_CB_PART ) ) ? 1 : 0;
				if ( 0 != cb0x || 0 != cb0y ) {
	...
	public class QCD {
		...
		public String toString ( ) {
			...
			if ( Markers.SQCX_NO_QUANTIZATION == qt )
				str += "No quantization \n";
			else if ( Markers.SQCX_SCALAR_DERIVED == qt )
				str += "Scalar derived\n";
			else if ( Markers.SQCX_SCALAR_EXPOUNDED == qt )
