...
public class BitOutputBuffer {
	...
	public final void writeBit ( final int bit ) {
		...
		if ( ( byte ) 0xFF != buf[ curbyte ] ) { // We don't need bit stuffing
			this.avbits = 8;
		}
	...
	public final void writeBits ( final int bits , int n ) {
		...
		if ( n >= this.avbits ) {
			...
			while ( n >= this.avbits ) {
				...
				if ( ( byte ) 0xFF != buf[ curbyte ] ) { // We don't need bit
					// stuffing
					this.avbits = 8;
				}
				else { // We need to stuff a bit (next MSBit is 0)
	...
	public final int getLength ( ) {
		if ( 8 == avbits ) { // A integral number of bytes
			return this.curbyte;
		}
	 ...
	 *
	 * @param data If non-null this array is used to return the data, which mus
