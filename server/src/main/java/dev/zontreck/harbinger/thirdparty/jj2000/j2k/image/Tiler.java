...
public class Tiler extends ImgDataAdapter implements BlkImgDataSrc {
	 ...
	 *
	 * @param src The 'BlkImgDataSrc' source from where to get the image data.
	 ...
	 * @param nw  The nominal tile width, on the reference grid. If 0 then there
	 *            is no tiling in that direction.
	 * @param nh  The nominal tile height, on the reference grid. If 0 then
	 *            there is no tiling in that direction.
	 * @throws IllegalArgumentException If src is tiled or "canvased", or if the arguments do not
	...
	public final int nextTile ( ) {
		int tIdx = 0;
		if ( this.tx == this.ntX - 1 && this.ty == this.ntY - 1 ) { // Already at last tile
			throw new NoNextElementException ( );
		}
		else if ( this.tx < this.ntX - 1 ) { // If not at end of current tile line
