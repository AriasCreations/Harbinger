...
import dev.zontreck.harbinger.thirdparty.jj2000.disp.*;
...
import dev.zontreck.harbinger.thirdparty.jj2000.j2k.image.output.ImgWriter;
import dev.zontreck.harbinger.thirdparty.jj2000.j2k.image.output.ImgWriterPGM;
import dev.zontreck.harbinger.thirdparty.jj2000.j2k.image.output.ImgWriterPGX;
import dev.zontreck.harbinger.thirdparty.jj2000.j2k.image.output.ImgWriterPPM;
import dev.zontreck.harbinger.thirdparty.jj2000.j2k.io.BEBufferedRandomAccessFile;
...
public class Decoder extends ImgDecoder implements Runnable {
	 ...
	 *            values).
	 * @param isp The component where the image is to be displayed if not output
	...
	public void run ( ) {
		...
		try {
			...
			if ( 1 <= infile.indexOf ( '/' ) && ':' == infile.charAt ( infile.indexOf ( '/' ) - 1 ) ) { // an URL
				final URL inurl;
				...
				if ( - 1 != datalen ) { // known length => initialize to length
			...
			if ( disp ) {
				...
				if ( 1 < nCompImg && ! ".PPM".equalsIgnoreCase ( outext ) ) { // Multiple file output
					// files
	...
	private void printUsage ( ) {
		...
		ml.println ( "dev.zontreck.harbinger.thirdparty.jj2000.JJ2KDecoder args...\n" , 10 , 12 );
