...
import dev.zontreck.harbinger.thirdparty.jj2000.j2k.io.*;
...
//	private static final int TILE_RESET = ~(PLM_FOUND | SIZ_FOUND | RGN_FOUND);
	...
	private void readQCD(final DataInputStream ehs, final boolean mainh, final int tileIdx, final int tpIdx) throws IOException
	{
		...
		if (SQCX_NO_QUANTIZATION == qType)
		{
			final int maxrl = (mainh ? ((Integer) this.decSpec.dls.getDefault()).intValue() : ((Integer) this.decSpec.dls
		...
		{
			final int maxrl = (SQCX_SCALAR_DERIVED == qType) ? 0 : (mainh ? ((Integer) this.decSpec.dls.getDefault()).intValue()
	...
	private void readQCC(final DataInputStream ehs, final boolean mainh, final int tileIdx, final int tpIdx) throws IOException
	{
		...
		if (SQCX_NO_QUANTIZATION == qType)
		{
			final int maxrl = (mainh ? ((Integer) this.decSpec.dls.getCompDef(cComp)).intValue() : ((Integer) this.decSpec.dls
		...
		{
			final int maxrl = (SQCX_SCALAR_DERIVED == qType) ? 0 : (mainh ? ((Integer) this.decSpec.dls.getCompDef(cComp))
	...
	private void readCOD(final DataInputStream ehs, final boolean mainh, final int tileIdx, final int tpIdx) throws IOException
	{
		...
		if (0 != (cstyle & SCOX_PRECINCT_PARTITION))
		{
		...
		if (0 != (cstyle & (SCOX_HOR_CB_PART | SCOX_VER_CB_PART)))
		{
			FacilityManager.getMsgLogger().printmsg(
					MsgLogger.WARNING,
		...
		if (0 != (ecOptions & ~(OPT_BYPASS | OPT_RESET_MQ | OPT_TERM_PASS | OPT_VERT_STR_CAUSAL | OPT_PRED_TERM | OPT_SEG_SYMBOLS)))
		{
...
*/
	 ...
	 *                if image contains a bad dev.zontreck.harbinger.thirdparty.jj2000.j2k.icc profile
	 * @exception ColorSpaceException
	 *                if image contains a bad dev.zontreck.harbinger.thirdparty.jj2000.j2k.colorspace box
	 */
