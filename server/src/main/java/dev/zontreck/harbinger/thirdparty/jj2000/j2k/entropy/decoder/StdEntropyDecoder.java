...
public class StdEntropyDecoder extends EntropyDecoder implements StdEntropyCoderOptions {
	 ...
	 */
	private static final int[] ZC_LUT_LH = new int[ 1 << StdEntropyDecoder.ZC_LUT_BITS ];
	/**
	 * Zero Coding context lookup tables for the HL global orientation
	 */
	private static final int[] ZC_LUT_HL = new int[ 1 << StdEntropyDecoder.ZC_LUT_BITS ];
	/**
	 * Zero Coding context lookup tables for the HH global orientation
	 */
	private static final int[] ZC_LUT_HH = new int[ 1 << StdEntropyDecoder.ZC_LUT_BITS ];
	/**
	 ...
	 * "sign predictor".
	 */
	private static final int[] SC_LUT = new int[ 1 << StdEntropyDecoder.SC_LUT_BITS ];
	/**
	 ...
	 */
	private static final int[] MR_LUT = new int[ 1 << StdEntropyDecoder.MR_LUT_BITS ];
	/**
	 ...
	 */
	private static final int STATE_SIG_R2 = StdEntropyDecoder.STATE_SIG_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The mask to isolate the significance bits for row 1 and 2 of the state
	 * array.
	 */
	private static final int SIG_MASK_R1R2 = StdEntropyDecoder.STATE_SIG_R1 | StdEntropyDecoder.STATE_SIG_R2;
	/**
	 * The flag bit for the "visited" bit in the state array, for row 2.
	 */
	private static final int STATE_VISITED_R2 = StdEntropyDecoder.STATE_VISITED_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The mask to isolate the visited bits for row 1 and 2 of the state array.
	 */
	private static final int VSTD_MASK_R1R2 = StdEntropyDecoder.STATE_VISITED_R1 | StdEntropyDecoder.STATE_VISITED_R2;
	/**
	 * The flag bit for the "not zero context" bit in the state array, for row
	 ...
	 * and STATE_D_DR_R2.
	 */
	private static final int STATE_NZ_CTXT_R2 = StdEntropyDecoder.STATE_NZ_CTXT_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 ...
	 */
	private static final int STATE_H_L_SIGN_R2 = StdEntropyDecoder.STATE_H_L_SIGN_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 ...
	 */
	private static final int STATE_H_R_SIGN_R2 = StdEntropyDecoder.STATE_H_R_SIGN_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 ...
	 */
	private static final int STATE_V_U_SIGN_R2 = StdEntropyDecoder.STATE_V_U_SIGN_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 ...
	 */
	private static final int STATE_V_D_SIGN_R2 = StdEntropyDecoder.STATE_V_D_SIGN_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The flag bit for the previous MR primitive applied in the state array,
	 * for row 2.
	 */
	private static final int STATE_PREV_MR_R2 = StdEntropyDecoder.STATE_PREV_MR_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The flag bit for the horizontal-left significance in the state array, for
	 * row 2.
	 */
	private static final int STATE_H_L_R2 = StdEntropyDecoder.STATE_H_L_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The flag bit for the horizontal-right significance in the state array,
	 * for row 2.
	 */
	private static final int STATE_H_R_R2 = StdEntropyDecoder.STATE_H_R_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The flag bit for the vertical-up significance in the state array, for row
	 * 2.
	 */
	private static final int STATE_V_U_R2 = StdEntropyDecoder.STATE_V_U_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The flag bit for the vertical-down significance in the state array, for
	 * row 2.
	 */
	private static final int STATE_V_D_R2 = StdEntropyDecoder.STATE_V_D_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The flag bit for the diagonal up-left significance in the state array,
	 * for row 2.
	 */
	private static final int STATE_D_UL_R2 = StdEntropyDecoder.STATE_D_UL_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The flag bit for the diagonal up-right significance in the state array,
	 * for row 2.
	 */
	private static final int STATE_D_UR_R2 = StdEntropyDecoder.STATE_D_UR_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The flag bit for the diagonal down-left significance in the state array,
	 * for row 2.
	 */
	private static final int STATE_D_DL_R2 = StdEntropyDecoder.STATE_D_DL_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 * The flag bit for the diagonal down-right significance in the state array
	 * , for row 2.
	 */
	private static final int STATE_D_DR_R2 = StdEntropyDecoder.STATE_D_DR_R1 << StdEntropyDecoder.STATE_SEP;
	/**
	 ...
	 * for row 2.
	 */
	private static final int SC_SHIFT_R2 = StdEntropyDecoder.SC_SHIFT_R1 + StdEntropyDecoder.STATE_SEP;
	/**
	 * The bit mask to isolate the state bits relative to the sign coding lookup
	 * table ('SC_LUT').
	 */
	private static final int SC_MASK = ( 1 << StdEntropyDecoder.SC_LUT_BITS ) - 1;
	/**
	...
	static {
		...
		for ( i = 0; 16 > i ; i++ ) {
			...
			StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_V_U_R1 | i ] = 5;
			StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_V_D_R1 | i ] = 5;
			// The two vertical coeffs significant
			StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_V_U_R1 | StdEntropyDecoder.STATE_V_D_R1 | i ] = 6;
		}
		// - One horiz. neighbor significant, diagonal/vertical non-significant
		StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_H_L_R1 ] = 7;
		StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_H_R_R1 ] = 7;
		// - One horiz. significant, no vertical significant, one or more
		// diagonal significant
		for ( i = 1; 16 > i ; i++ ) {
			StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_H_L_R1 | i ] = 8;
			StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_H_R_R1 | i ] = 8;
		}
		// - One horiz. significant, one or more vertical significant,
		// diagonal irrelevant
		for ( i = 1; 4 > i ; i++ ) {
			for ( j = 0; 16 > j ; j++ ) {
				StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_H_L_R1 | ( i << 4 ) | j ] = 9;
				StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_H_R_R1 | ( i << 4 ) | j ] = 9;
			}
		}
		// - Two horiz. significant, others irrelevant
		for ( i = 0; 64 > i ; i++ ) {
			StdEntropyDecoder.ZC_LUT_LH[ StdEntropyDecoder.STATE_H_L_R1 | StdEntropyDecoder.STATE_H_R_R1 | i ] = 10;
		}
		...
		for ( i = 0; 16 > i ; i++ ) {
			...
			StdEntropyDecoder.ZC_LUT_HL[ StdEntropyDecoder.STATE_H_L_R1 | i ] = 5;
			StdEntropyDecoder.ZC_LUT_HL[ StdEntropyDecoder.STATE_H_R_R1 | i ] = 5;
			// Two horiz. significant
			StdEntropyDecoder.ZC_LUT_HL[ StdEntropyDecoder.STATE_H_L_R1 | StdEntropyDecoder.STATE_H_R_R1 | i ] = 6;
		}
		// - One vert. significant, diagonal/horizontal non-significant
		StdEntropyDecoder.ZC_LUT_HL[ StdEntropyDecoder.STATE_V_U_R1 ] = 7;
		StdEntropyDecoder.ZC_LUT_HL[ StdEntropyDecoder.STATE_V_D_R1 ] = 7;
		// - One vert. significant, horizontal non-significant, one or more
		// diag. significant
		for ( i = 1; 16 > i ; i++ ) {
			StdEntropyDecoder.ZC_LUT_HL[ StdEntropyDecoder.STATE_V_U_R1 | i ] = 8;
			StdEntropyDecoder.ZC_LUT_HL[ StdEntropyDecoder.STATE_V_D_R1 | i ] = 8;
		}
		// - One vertical significant, one or more horizontal significant,
		// diagonal irrelevant
		for ( i = 1; 4 > i ; i++ ) {
			for ( j = 0; 16 > j ; j++ ) {
				StdEntropyDecoder.ZC_LUT_HL[ ( i << 6 ) | StdEntropyDecoder.STATE_V_U_R1 | j ] = 9;
				StdEntropyDecoder.ZC_LUT_HL[ ( i << 6 ) | StdEntropyDecoder.STATE_V_D_R1 | j ] = 9;
			}
		}
		// - Two vertical significant, others irrelevant
		for ( i = 0; 4 > i ; i++ ) {
			for ( j = 0; 16 > j ; j++ ) {
				StdEntropyDecoder.ZC_LUT_HL[ ( i << 6 ) | StdEntropyDecoder.STATE_V_U_R1 | StdEntropyDecoder.STATE_V_D_R1 | j ] = 10;
			}
		}
		...
		inter_sc_lut[ ( 1 << 3 ) | 0 ] = 12 | StdEntropyDecoder.INT_SIGN_BIT;
		inter_sc_lut[ ( 0 << 3 ) | 2 ] = 13 | StdEntropyDecoder.INT_SIGN_BIT;
		inter_sc_lut[ ( 0 << 3 ) | 1 ] = 14 | StdEntropyDecoder.INT_SIGN_BIT;
		inter_sc_lut[ ( 0 << 3 ) | 0 ] = 15 | StdEntropyDecoder.INT_SIGN_BIT;
		...
		for ( i = 0; ( 1 << StdEntropyDecoder.SC_LUT_BITS ) - 1 > i ; i++ ) {
			...
			StdEntropyDecoder.SC_LUT[ i ] = inter_sc_lut[ ( h + 1 ) << 3 | ( v + 1 ) ];
		}
		inter_sc_lut = null;
		...
		for ( i = 1; ( 1 << ( StdEntropyDecoder.MR_LUT_BITS - 1 ) ) > i ; i++ ) {
			StdEntropyDecoder.MR_LUT[ i ] = 17;
		}
		// Previous MR on, significance irrelevant
		for ( ; ( 1 << StdEntropyDecoder.MR_LUT_BITS ) > i ; i++ ) {
...
//			| STATE_NZ_CTXT_R1 | STATE_NZ_CTXT_R2;
	 ...
	 * @param verber This flag indicates if the entropy decoder should be verbose
	 *               about bit stream errors that are detected and concealed.
	 * @param mQuit  the maximum number of bit planes to decode according to the m
	 *               quit condition
	 */
	@SuppressWarnings ("deprecation")
	public StdEntropyDecoder ( final CodedCBlkDataSrcDec src , final DecoderSpecs decSpec , final boolean doer , final boolean verber , final int mQuit ) {
		super ( src );
		...
		if ( StdEntropyDecoder.DO_TIMING ) {
	 ...
	 */
	@Override
	public DataBlk getCodeBlock ( final int c , final int m , final int n , final SubbandSyn sb , DataBlk cblk ) {
		...
		final int tslen; // Length of first terminated segment
		...
		if ( null == out_data || out_data.length < this.srcblk.w * this.srcblk.h ) {
		...
		if ( 0 >= srcblk.nl || 0 >= srcblk.nTrunc ) {
			// 0 layers => no data to decode => return all 0s
			return cblk;
		}

		// Get the length of the first terminated segment
		tslen = ( null == srcblk.tsLengths ) ? this.srcblk.dl : this.srcblk.tsLengths[ 0 ];
		tsidx = 0;
		// Initialize for decoding
		npasses = this.srcblk.nTrunc;
		if ( null == mq ) {
			in = new ByteInputBuffer ( this.srcblk.data , 0 , tslen );
			this.mq = new MQDecoder ( in , StdEntropyDecoder.NUM_CTXTS , StdEntropyDecoder.MQ_INIT );
		}
		else {
		...
		if ( 0 != ( options & StdEntropyCoderOptions.OPT_BYPASS ) ) {
			if ( null == bin ) {
				if ( null == in )
		...
		if ( - 1 != mQuit && ( this.mQuit * 3 - 2 ) < npasses ) {
			npasses = this.mQuit * 3 - 2;
		}

		// First bit-plane has only the cleanup pass
		if ( 0 <= curbp && 0 < npasses ) {
			isterm = 0 != ( options & StdEntropyCoderOptions.OPT_TERM_PASS )
					|| ( 0 != ( options & StdEntropyCoderOptions.OPT_BYPASS ) && ( 31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - this.srcblk.skipMSBP ) >= curbp );
			error = this.cleanuppass ( cblk , this.mq , curbp , this.state , zc_lut , isterm );
			npasses--;
			if ( ! error || ! this.doer )
				curbp--;
		}
		...
		if ( ! error || ! this.doer ) {
			while ( 0 <= curbp && 0 < npasses ) {

				if ( 0 != ( options & StdEntropyCoderOptions.OPT_BYPASS ) && ( curbp < 31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - this.srcblk.skipMSBP ) ) {
					...
					npasses--;
					if ( 0 >= npasses || ( error && this.doer ) )
						break;

					if ( 0 != ( options & StdEntropyCoderOptions.OPT_TERM_PASS ) ) {
					...
					isterm = 0 != ( options & StdEntropyCoderOptions.OPT_TERM_PASS );
					error = this.sigProgPass ( cblk , this.mq , curbp , this.state , zc_lut , isterm );
					npasses--;
					if ( 0 >= npasses || ( error && this.doer ) )
						break;
				...
				if ( 0 != ( options & StdEntropyCoderOptions.OPT_TERM_PASS )
						|| ( 0 != ( options & StdEntropyCoderOptions.OPT_BYPASS ) && ( curbp < 31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - this.srcblk.skipMSBP ) ) ) {
					// Here starts a new MQ segment
					++ tsidx;
					this.mq.nextSegment ( null , - 1 , this.srcblk.tsLengths[ tsidx ] );
				}
	 ...
	 */
	@Override
	public DataBlk getInternCodeBlock ( final int c , final int m , final int n , final SubbandSyn sb , final DataBlk cblk ) {
	 ...
	 */
	private boolean sigProgPass ( final DataBlk cblk , final MQDecoder mq , final int bp , final int[] state , final int[] zc_lut , final boolean isterm ) {
		...
		final int setmask; // The mask to set current and lower bit-planes to 1/2
		// approximation
		...
		for ( s = nstripes - 1; 0 <= s ; s-- , sk += kstep , sj += jstep ) {
			sheight = ( 0 != s ) ? StdEntropyCoderOptions.STRIPE_HEIGHT : cblk.h - ( nstripes - 1 ) * StdEntropyCoderOptions.STRIPE_HEIGHT;
			stopsk = sk + cblk.w;
			...
			for ( ; sk < stopsk ; sk++ , sj++ ) {
				...
				if ( 0 != ( ( ( ~ csj ) & ( csj << 2 ) ) & SIG_MASK_R1R2 ) ) {
					...
					if ( STATE_NZ_CTXT_R1 == ( csj & ( STATE_SIG_R1 | STATE_NZ_CTXT_R1 ) ) ) {
						// Use zero coding
						if ( 0 != mq.decodeSymbol ( zc_lut[ csj & ZC_MASK ] ) ) {
							// Became significant
							// Use sign coding
							ctxt = StdEntropyDecoder.SC_LUT[ ( csj >>> StdEntropyDecoder.SC_SHIFT_R1 ) & StdEntropyDecoder.SC_MASK ];
							sym = mq.decodeSymbol ( ctxt & StdEntropyDecoder.SC_LUT_MASK ) ^ ( ctxt >>> StdEntropyDecoder.SC_SPRED_SHIFT );
							// Update data
							...
							if ( ! causal ) {
								...
								// previous stripe.
								state[ j + off_ul ] |= StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DR_R2;
								state[ j + off_ur ] |= StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DL_R2;
							}
							// Update sign state information of neighbors
							if ( 0 != sym ) {
								csj |= StdEntropyDecoder.STATE_SIG_R1 | StdEntropyDecoder.STATE_VISITED_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_V_U_R2
										| StdEntropyDecoder.STATE_V_U_SIGN_R2;
								if ( ! causal ) {
									// If in causal mode do not change
									// contexts of previous stripe.
									state[ j - sscanw ] |= StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_V_D_R2 | StdEntropyDecoder.STATE_V_D_SIGN_R2;
								}
								state[ j + 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_H_L_R1 | StdEntropyDecoder.STATE_H_L_SIGN_R1
										| StdEntropyDecoder.STATE_D_UL_R2;
								state[ j - 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_H_R_R1 | StdEntropyDecoder.STATE_H_R_SIGN_R1
										| StdEntropyDecoder.STATE_D_UR_R2;
							}
							else {
								csj |= StdEntropyDecoder.STATE_SIG_R1 | StdEntropyDecoder.STATE_VISITED_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_V_U_R2;
								if ( ! causal ) {
									// If in causal mode do not change
									// contexts of previous stripe.
									state[ j - sscanw ] |= StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_V_D_R2;
								}
								state[ j + 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_H_L_R1 | StdEntropyDecoder.STATE_D_UL_R2;
								state[ j - 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_H_R_R1 | StdEntropyDecoder.STATE_D_UR_R2;
							}
						}
						else {
							csj |= StdEntropyDecoder.STATE_VISITED_R1;
						}
					}
					if ( 2 > sheight ) {
						state[ j ] = csj;
					...
					if ( STATE_NZ_CTXT_R2 == ( csj & ( STATE_SIG_R2 | STATE_NZ_CTXT_R2 ) ) ) {
						k += dscanw;
						// Use zero coding
						if ( 0 != mq.decodeSymbol ( zc_lut[ ( csj >>> STATE_SEP ) & ZC_MASK ] ) ) {
							// Became significant
							// Use sign coding
							ctxt = StdEntropyDecoder.SC_LUT[ ( csj >>> StdEntropyDecoder.SC_SHIFT_R2 ) & StdEntropyDecoder.SC_MASK ];
							sym = mq.decodeSymbol ( ctxt & StdEntropyDecoder.SC_LUT_MASK ) ^ ( ctxt >>> StdEntropyDecoder.SC_SPRED_SHIFT );
							// Update data
							data[ k ] = ( sym << 31 ) | setmask;
							// Update state information (significant bit,
							...
							// neighbors, non zero context of neighbors, sign
							// of neighbors)
							state[ j + off_dl ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_D_UR_R1;
							state[ j + off_dr ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_D_UL_R1;
							// Update sign state information of neighbors
							if ( 0 != sym ) {
								csj |= StdEntropyDecoder.STATE_SIG_R2 | StdEntropyDecoder.STATE_VISITED_R2 | StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_V_D_R1
										| StdEntropyDecoder.STATE_V_D_SIGN_R1;
								state[ j + sscanw ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_V_U_R1 | StdEntropyDecoder.STATE_V_U_SIGN_R1;
								state[ j + 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DL_R1 | StdEntropyDecoder.STATE_H_L_R2
										| StdEntropyDecoder.STATE_H_L_SIGN_R2;
								state[ j - 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DR_R1 | StdEntropyDecoder.STATE_H_R_R2
										| StdEntropyDecoder.STATE_H_R_SIGN_R2;
							}
							else {
								csj |= StdEntropyDecoder.STATE_SIG_R2 | StdEntropyDecoder.STATE_VISITED_R2 | StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_V_D_R1;
								state[ j + sscanw ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_V_U_R1;
								state[ j + 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DL_R1 | StdEntropyDecoder.STATE_H_L_R2;
								state[ j - 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DR_R1 | StdEntropyDecoder.STATE_H_R_R2;
							}
						}
						else {
							csj |= StdEntropyDecoder.STATE_VISITED_R2;
						}
					}
					state[ j ] = csj;
				}
		...
		if ( isterm && 0 != ( options & StdEntropyCoderOptions.OPT_PRED_TERM ) ) {
	 ...
	 * @return True if an error was detected in the bit stream, false otherwise.
	 */
	private boolean rawSigProgPass ( final DataBlk cblk , final ByteToBitInput bin , final int bp , final int[] state , final boolean isterm ) {
		int j, sj; // The state index for line and stripe
		int k, sk; // The data index for line and stripe
		final int dscanw; // The data scan-width
		final int sscanw; // The state scan-width
	 ...
	 * @return True if an error was detected in the bit stream, false otherwise.
	 */
	private boolean magRefPass ( final DataBlk cblk , final MQDecoder mq , final int bp , final int[] state , final boolean isterm ) {
		int j, sj; // The state index for line and stripe
		int k, sk; // The data index for line and stripe
		final int dscanw; // The data scan-width
		final int sscanw; // The state scan-width
		...
		int stopsk; // The loop limit on the variable sk
		int csj; // Local copy (i.e. cached) of 'state[j]'
		final int setmask; // The mask to set lower bit-planes to 1/2 approximation
		final int resetmask; // The mask to reset approximation bit-planes
			...
			for ( ; sk < stopsk ; sk++ , sj++ ) {
				...
				if ( 0 != ( ( ( csj >>> 1 ) & ( ~ csj ) ) & VSTD_MASK_R1R2 ) ) {
					...
					if ( STATE_SIG_R1 == ( csj & ( STATE_SIG_R1 | STATE_VISITED_R1 ) ) ) {
						// Use MR primitive
						sym = mq.decodeSymbol ( StdEntropyDecoder.MR_LUT[ csj & StdEntropyDecoder.MR_MASK ] );
						// Update the data
						data[ k ] &= resetmask;
						data[ k ] |= ( sym << bp ) | setmask;
						// Update the STATE_PREV_MR bit
						csj |= StdEntropyDecoder.STATE_PREV_MR_R1;
					}
					if ( 2 > sheight ) {
						state[ j ] = csj;
						continue;
					...
					if ( STATE_SIG_R2 == ( csj & ( STATE_SIG_R2 | STATE_VISITED_R2 ) ) ) {
						k += dscanw;
						// Use MR primitive
						sym = mq.decodeSymbol ( StdEntropyDecoder.MR_LUT[ ( csj >>> StdEntropyDecoder.STATE_SEP ) & StdEntropyDecoder.MR_MASK ] );
						// Update the data
						data[ k ] &= resetmask;
						data[ k ] |= ( sym << bp ) | setmask;
						// Update the STATE_PREV_MR bit
						csj |= StdEntropyDecoder.STATE_PREV_MR_R2;
					}
					state[ j ] = csj;
				}
				// Do half bottom of column
	 ...
	 * @return True if an error was detected in the bit stream, false otherwise.
	 */
	private boolean rawMagRefPass ( final DataBlk cblk , final ByteToBitInput bin , final int bp , final int[] state , final boolean isterm ) {
		int j, sj; // The state index for line and stripe
		int k, sk; // The data index for line and stripe
		final int dscanw; // The data scan-width
		final int sscanw; // The state scan-width
	 ...
	 * @return True if an error was detected in the bit stream, false otherwise.
	 */
	private boolean cleanuppass ( final DataBlk cblk , final MQDecoder mq , final int bp , final int[] state , final int[] zc_lut , final boolean isterm ) {
		int j, sj; // The state index for line and stripe
		int k, sk; // The data index for line and stripe
		final int dscanw; // The data scan-width
		final int sscanw; // The state scan-width
			...
			for ( ; sk < stopsk ; sk++ , sj++ ) {
				...
				top_half:
				{
					...
					if ( 0 == csj && 0 == state[ j + sscanw ] && StdEntropyCoderOptions.STRIPE_HEIGHT == sheight ) {
						if ( 0 != mq.decodeSymbol ( RLC_CTXT ) ) {
							// run-length is significant, decode length
							rlclen = mq.decodeSymbol ( StdEntropyDecoder.UNIF_CTXT ) << 1;
							rlclen |= mq.decodeSymbol ( StdEntropyDecoder.UNIF_CTXT );
						...
						if ( 0 == ( rlclen & 0x01 ) ) {
							...
							// Update sign state information of neighbors
							if ( 0 != sym ) {
								csj |= StdEntropyDecoder.STATE_SIG_R2 | StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_V_D_R1 | StdEntropyDecoder.STATE_V_D_SIGN_R1;
								state[ j + sscanw ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_V_U_R1 | StdEntropyDecoder.STATE_V_U_SIGN_R1;
								state[ j + 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DL_R1 | StdEntropyDecoder.STATE_H_L_R2
										| StdEntropyDecoder.STATE_H_L_SIGN_R2;
								state[ j - 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DR_R1 | StdEntropyDecoder.STATE_H_R_R2
								...
								csj |= StdEntropyDecoder.STATE_SIG_R2 | StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_V_D_R1;
								state[ j + sscanw ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_V_U_R1;
								state[ j + 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DL_R1 | StdEntropyDecoder.STATE_H_L_R2;
								state[ j - 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_D_DR_R1 | StdEntropyDecoder.STATE_H_R_R2;
							}
					...
					if ( VSTD_MASK_R1R2 != ( ( ( csj >> 1 ) | csj ) & VSTD_MASK_R1R2 ) ) {
						...
						if ( 0 == ( csj & ( STATE_SIG_R1 | STATE_VISITED_R1 ) ) ) {
							...
							if ( 0 != mq.decodeSymbol ( zc_lut[ csj & ZC_MASK ] ) ) {
								...
								if ( 0 != sym ) {
									...
									state[ j + 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_H_L_R1
											| StdEntropyDecoder.STATE_H_L_SIGN_R1 | StdEntropyDecoder.STATE_D_UL_R2;
									state[ j - 1 ] |= StdEntropyDecoder.STATE_NZ_CTXT_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_H_R_R1
											| StdEntropyDecoder.STATE_H_R_SIGN_R1 | StdEntropyDecoder.STATE_D_UR_R2;
								}
								else {
									csj |= StdEntropyDecoder.STATE_SIG_R1 | StdEntropyDecoder.STATE_VISITED_R1 | StdEntropyDecoder.STATE_NZ_CTXT_R2 | StdEntropyDecoder.STATE_V_U_R2;
									if ( ! causal ) {
						...
						if ( 2 > sheight ) {
							csj &= ~ ( StdEntropyDecoder.STATE_VISITED_R1 | StdEntropyDecoder.STATE_VISITED_R2 );
							state[ j ] = csj;
							continue;
						}
						// Scan second row
		...
		if ( 0 != ( options & StdEntropyCoderOptions.OPT_SEG_SYMBOLS ) ) {
			sym = mq.decodeSymbol ( StdEntropyDecoder.UNIF_CTXT ) << 3;
			sym |= mq.decodeSymbol ( StdEntropyDecoder.UNIF_CTXT ) << 2;
			sym |= mq.decodeSymbol ( StdEntropyDecoder.UNIF_CTXT ) << 1;
			sym |= mq.decodeSymbol ( StdEntropyDecoder.UNIF_CTXT );
			// Set error condition accordingly
			error = SEG_SYMBOL != sym;
		}
	 ...
	 */
	private void conceal ( final DataBlk cblk , final int bp ) {
		int l; // line index
		int k; // array index
		int kmax; // 'k' limit
		int dk; // Value of data[k]
		final int[] data; // the data array
		final int setmask; // Bitmask to set approximation to 1/2 of
		// known interval on significant data
		final int resetmask; // Bitmask to erase all the data from
		// bit-plane 'bp'
		...
		for ( l = cblk.h - 1 , k = cblk.offset; 0 <= l ; l-- ) {
			for ( kmax = k + cblk.w; k < kmax ; k++ ) {
				dk = data[ k ];
				if ( 0 != ( dk & resetmask & 0x7FFFFFFF ) ) {
