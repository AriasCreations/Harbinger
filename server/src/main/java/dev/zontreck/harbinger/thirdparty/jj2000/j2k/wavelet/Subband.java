 ...
 *
 * @see dev.zontreck.harbinger.thirdparty.jj2000.j2k.wavelet.analysis.SubbandAn
 * @see dev.zontreck.harbinger.thirdparty.jj2000.j2k.wavelet.synthesis.SubbandSyn
 */
public abstract class Subband {
	 ...
	 * @param hfilters The horizontal wavelet filters (analysis or synthesis) for
	 ...
	 *                 levels.
	 * @param vfilters The vertical wavelet filters (analysis or synthesis) for each
	 ...
	 */
	protected Subband ( final int w , final int h , final int ulcx , final int ulcy , final int lvls , final WaveletFilter[] hfilters , final WaveletFilter[] vfilters ) {
		int i, hi, vi;
	...
	protected void initChilds ( ) {
		final Subband subb_LL = this.getLL ( );
		final Subband subb_HL = this.getHL ( );
		final Subband subb_LH = this.getLH ( );
		final Subband subb_HH = this.getHH ( );
		...
		subb_LL.resLvl = ( WT_ORIENT_LL == orientation ) ? this.resLvl - 1 : this.resLvl;
		subb_LL.anGainExp = this.anGainExp;
		subb_LL.sbandIdx = ( this.sbandIdx << 2 );
		// HL subband
		subb_HL.orientation = Subband.WT_ORIENT_HL;
		...
		// LH subband
		subb_LH.orientation = Subband.WT_ORIENT_LH;
		...
		// HH subband
		subb_HH.orientation = Subband.WT_ORIENT_HH;
	...
	public Subband nextSubband ( ) {
		...
		switch ( this.orientation ) {
			case Subband.WT_ORIENT_LL:
				sb = this.getParent ( );
				if ( null == sb || sb.resLvl != this.resLvl ) {
			...
			case Subband.WT_ORIENT_HH:
				...
				sb = this;
				while ( WT_ORIENT_HH == sb.orientation ) {
					sb = sb.getParent ( );
				}
	...
	public Subband getNextResLevel ( ) {
		...
		if ( 0 == level ) { // No higher res. level
			return null;
		}
		...
		do {
			sb = sb.getParent ( );
			if ( null == sb ) { // No higher resolution level
				return null;
			}
	 ...
	 */
	public Subband getSubbandByIdx ( final int rl , final int sbi ) {
		Subband sb = this;

		// Find the root subband for the resolution level
		if ( rl > sb.resLvl || 0 > rl ) {
	 ...
	 */
	public Subband getSubband ( final int x , final int y ) {
		Subband cur, hhs;
	...
	public String toString ( ) {
		final String string = "w=" + this.w + ", h=" + this.h + ", ulx=" + this.ulx + ", uly=" + this.uly + ", ulcx=" + this.ulcx + ", ulcy=" + this.ulcy
				+ ", idx=" + this.sbandIdx + ", orient=" + this.orientation + ", node=" + this.isNode + ", level=" + this.level
				+ ", resLvl=" + this.resLvl + ", nomCBlkW=" + this.nomCBlkW + ", nomCBlkH=" + this.nomCBlkH + ", numCb=" + this.numCb;
