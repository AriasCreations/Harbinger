...
import dev.zontreck.harbinger.thirdparty.jj2000.j2k.entropy.decoder.DecLyrdCBlk;
...
//	private int firstTilePartHeadLen;
	 ...
	 * @see #allocateRate
	 */
	public FileBitstreamReaderAgent (
			final HeaderDecoder hd , final RandomAccessIO ehs , final DecoderSpecs decSpec , final ParameterList pl ,
			final boolean cdstrInfo , final HeaderInfo hi
	) throws IOException {
		super ( hd , decSpec );
		...
		try {
			while ( 0 != remainingTileParts ) {
				...
				try {
					...
					if ( this.isEOCFound ) { // Some tiles are missing but the
						// codestream is OK
						break;
					}
					tp = this.tilePartsRead[ t ];
					if ( this.isPsotEqualsZero ) { // Psot may equals zero for the
...
//				tptot++;
		...
		if ( ! this.isEOCFound && ! this.isPsotEqualsZero ) {
			try {
				if ( ! rateReached && ! this.isPsotEqualsZero && Markers.EOC != in.readShort ( ) ) {
					FacilityManager.getMsgLogger ( ).printmsg (
							MsgLogger.WARNING ,
	...
	private int readTilePartHeader ( ) throws IOException {
		...
		if ( Markers.SOT != marker ) {
			if ( Markers.EOC == marker ) {
		...
		if ( 0 == nrOfTileParts ) { // The number of tile-part is not specified in
			...
			if ( 0 == tileParts[ tile ] ) { // First tile-part: OK
	...
	public int setTile ( final int x , final int y ) {
		...
		for ( int c = 0 ; c < this.nc ; c++ ) {
			...
			this.subbTrees[ c ] = new SubbandSyn ( this.getTileCompWidth ( t , c , this.mdl[ c ] ) , this.getTileCompHeight ( t , c , this.mdl[ c ] ) , this.getResULX (
					c ,
					this.mdl[ c ]
			) , this.getResULY ( c , this.mdl[ c ] ) , this.mdl[ c ] , this.decSpec.wfs.getHFilters ( t , c ) , this.decSpec.wfs.getVFilters ( t , c ) );
	...
	public int nextTile ( ) {
		int tIdx = 0;
		if ( this.ctX == this.ntX - 1 && this.ctY == this.ntY - 1 ) { // Already at last tile
			throw new NoNextElementException ( );
		}
		else if ( this.ctX < this.ntX - 1 ) { // If not at end of current tile line
	...
	public DecLyrdCBlk getCodeBlock ( final int c , final int m , final int n , final SubbandSyn sb , final int fl , int nl , DecLyrdCBlk ccb ) {
		...
		if ( 0 != ( options & StdEntropyCoderOptions.OPT_TERM_PASS ) ) {
		...
		else if ( 0 != ( options & StdEntropyCoderOptions.OPT_BYPASS ) ) {
			...
			if ( StdEntropyCoderOptions.FIRST_BYPASS_PASS_IDX >= ccb.nTrunc ) {
				nts = 1;
			}
			else {
				nts = 1;
				...
				for ( tpidx = ccb.ftpIdx; tpidx < ccb.nTrunc ; tpidx++ ) {
					if ( StdEntropyCoderOptions.FIRST_BYPASS_PASS_IDX - 1 <= tpidx ) {
		...
		else if ( 1 < nts && StdEntropyCoderOptions.OPT_BYPASS == ( options & ( StdEntropyCoderOptions.OPT_BYPASS | StdEntropyCoderOptions.OPT_TERM_PASS ) ) ) {
