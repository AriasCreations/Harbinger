...
import dev.zontreck.harbinger.thirdparty.jj2000.j2k.wavelet.analysis.CBlkWTDataSrc;
...
public abstract class Quantizer extends ImgDataAdapter implements CBlkQuantDataSrcEnc {
	 ...
	 */
	protected Quantizer ( final CBlkWTDataSrc src ) {
		super ( src );
		this.src = src;
	}
	 ...
	 */
	public static Quantizer createInstance ( final CBlkWTDataSrc src , final EncoderSpecs encSpec ) {
	...
	public SubbandAn getAnSubbandTree ( final int t , final int c ) {
		final SubbandAn sbba;

		// Ask for the wavelet tree of the source
		sbba = this.src.getAnSubbandTree ( t , c );
		// Calculate the stepWMSE
		this.calcSbParams ( sbba , c );
		return sbba;
	}
