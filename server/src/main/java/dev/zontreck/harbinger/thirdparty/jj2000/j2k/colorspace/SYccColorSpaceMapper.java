...
public class SYccColorSpaceMapper extends ColorSpaceMapper {
	...
	private static DataBlkFloat[] mult ( final DataBlkFloat[] inblk ) {
		...
		for ( j = 0; j < length ; ++ j ) {
			out[ 0 ][ j ] = (
					SYccColorSpaceMapper.Matrix00 * in[ 0 ][ inblk[ 0 ].offset + j ] + SYccColorSpaceMapper.Matrix01 * in[ 1 ][ inblk[ 1 ].offset + j ] + SYccColorSpaceMapper.Matrix02
							* in[ 2 ][ inblk[ 2 ].offset + j ]
			);

			out[ 1 ][ j ] = (
					SYccColorSpaceMapper.Matrix10 * in[ 0 ][ inblk[ 0 ].offset + j ] + SYccColorSpaceMapper.Matrix11 * in[ 1 ][ inblk[ 1 ].offset + j ] + SYccColorSpaceMapper.Matrix12
							* in[ 2 ][ inblk[ 2 ].offset + j ]
			);

			out[ 2 ][ j ] = (
					SYccColorSpaceMapper.Matrix20 * in[ 0 ][ inblk[ 0 ].offset + j ] + SYccColorSpaceMapper.Matrix21 * in[ 1 ][ inblk[ 1 ].offset + j ] + SYccColorSpaceMapper.Matrix22
							* in[ 2 ][ inblk[ 2 ].offset + j ]
			);
		}
	 ...
	 * input data.
	 *
	 * @param outblk Its coordinates and dimensions specify the area to return. If
