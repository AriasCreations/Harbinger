...
//			| STATE_NZ_CTXT_R1 | STATE_NZ_CTXT_R2;
	...
	public StdEntropyCoder(final CBlkQuantDataSrcEnc src, final CBlkSizeSpec cblks, final PrecinctSizeSpec pss, final StringSpec bms,
	...
	public CBlkRateDistStats getNextCodeBlock(final int c, CBlkRateDistStats ccb)
	{
		...
		{ // Use single threaded implementation
			...
			if (0 != (opts[tIdx][c] & OPT_BYPASS) && null == boutT[0])
			{
		...
		while (!this.finishedTileComponent[c] && !this.idleComps.empty())
		{
			...
			{
				// Initialize thread local variables
				if (0 != (opts[tIdx][c] & OPT_BYPASS) && null == boutT[cIdx])
				{
	 ...
	 */
	private static void compressCodeBlock(final int c, final CBlkRateDistStats ccb, final CBlkWTData srcblk, final MQCoder mq,
										  ...
										  final boolean[] istermbuf, final int[] symbuf, final int[] ctxtbuf, final int options, final boolean rev, final int lcType, final int tType)
	{
		...
		if (0 != (options & OPT_PRED_TERM) && MQCoder.TERM_PRED_ER != tType)
		{
		...
		if (curbp >= lmb)
		{
			...
			// bit-plane is "raw".
			istermbuf[npass] = 0 != (options & OPT_TERM_PASS) || curbp == lmb
					|| (0 != (options & OPT_BYPASS) && (31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - skipbp) >= curbp);
		...
		while (curbp >= lmb)
		{
			...
			istermbuf[npass] = 0 != (options & OPT_TERM_PASS);
			if (0 == (options & OPT_BYPASS) || (31 - StdEntropyCoderOptions.NUM_NON_BYPASS_MS_BP - skipbp <= curbp))
			{ // No bypass coding
			...
			{ // Bypass ("raw") coding
				bout.setPredTerm(0 != (options & OPT_PRED_TERM));
		...
		ccb.selectConvexHull(ratebuf, distbuf, 0 != (options & (OPT_BYPASS | OPT_TERM_PASS)) ? istermbuf : null, npass,
				rev);
	 ...
	 *         tables.
	 */
	private static int sigProgPass(final CBlkWTData srcblk, final MQCoder mq, final boolean doterm, final int bp, final int[] state, final int[] fs,
	 ...
	 *         tables.
	 */
	private static int rawSigProgPass(final CBlkWTData srcblk, final BitToByteOutput bout, final boolean doterm, final int bp, final int[] state,
...
//							nsym++;
	 ...
	 *         tables.
	 */
	private static int magRefPass(final CBlkWTData srcblk, final MQCoder mq, final boolean doterm, final int bp, final int[] state, final int[] fm,
	 ...
	 *         tables.
	 */
	private static int rawMagRefPass(final CBlkWTData srcblk, final BitToByteOutput bout, final boolean doterm, final int bp, final int[] state,
...
//						nsym++;
	 ...
	 *         tables.
	 */
	private static int cleanuppass(final CBlkWTData srcblk, final MQCoder mq, final boolean doterm, final int bp, final int[] state, final int[] fs,
								   final int[] zc_lut, final int[] symbuf, final int[] ctxtbuf, final int[] ratebuf, final int pidx, final int ltpidx, final int options)
	{
		...
		for (int t = 0; t < nt; t++)
		{
			for (int c = 0; c < nc; c++)
			{
				...
				else if ("predict".equalsIgnoreCase(termType))
				{
					...
					if (0 == (opts[t][c] & (OPT_TERM_PASS | OPT_BYPASS)))
					{
