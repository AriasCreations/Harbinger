...
public abstract class BufferedRandomAccessFile implements RandomAccessIO, EndianType {
	...
	protected BufferedRandomAccessFile ( final File file , String mode , final int bufferSize ) throws IOException {
		if ( "rw".equals ( mode ) || "rw+".equals ( mode ) ) { // mode read / write
			this.isReadOnly = false;
			if ( "rw".equals ( mode ) ) { // mode read / (over)write
	...
	protected final void readNewBuffer ( final int off ) throws IOException {
		...
		if ( this.maxByte < this.byteBuffer.length ) { // Not enough data in input file.
	...
	public final int read ( ) throws IOException {
		if ( this.pos < this.maxByte ) { // The byte can be read from the buffer
			// In Java, the bytes are always signed.
			int i = ( byteBuffer[ this.pos ] & 0xFF );
			this.pos++;
			return i;
		}
		else if ( this.isEOFInBuffer ) { // EOF is reached
	...
	public final void readFully ( final byte[] b , int off , int len ) throws IOException {
		...
		while ( 0 < len ) {
			...
			if ( this.pos < this.maxByte ) { // We can read some data from buffer
	 ...
	 * @throws java.io.EOFException If the end-of file was reached before all the bytes could
	 *                              be skipped.
	 * @throws java.io.IOException  If an I/O error occurred.
	 */
	@Override
