...
import dev.zontreck.harbinger.thirdparty.jj2000.j2k.io.BufferedRandomAccessFile;
...
public class ImgReaderPGX extends ImgReader implements EndianType {
	...
	public DataBlk getInternCompData ( DataBlk blk , final int c ) {
		...
		try {
			switch ( this.packBytes ) { // Switch between one of the 3 byte packet type
							...
							switch ( this.byteOrder ) {
								case EndianType.LITTLE_ENDIAN:
									for ( k = ( i - blk.uly ) * blk.w + blk.w - 1 , j = ( blk.w << 2 ) - 1; 0 <= j ; k-- ) {
										barr[ k ] = (
												(
														( ( this.buf[ j ] & 0xFF ) << 24 ) | ( ( this.buf[ j ] & 0xFF ) << 16 )
																| ( ( this.buf[ j ] & 0xFF ) << 8 ) | ( this.buf[ j ] & 0xFF )
												) << paddingLength
										) >> paddingLength;
										j--;
										j--;
								...
								case EndianType.BIG_ENDIAN:
									for ( k = ( i - blk.uly ) * blk.w + blk.w - 1 , j = ( blk.w << 2 ) - 1; 0 <= j ; k-- ) {
										barr[ k ] = (
												(
														( this.buf[ j ] & 0xFF ) | ( ( this.buf[ j ] & 0xFF ) << 8 )
																| ( ( this.buf[ j ] & 0xFF ) << 16 ) | ( ( this.buf[ j ] & 0xFF ) << 24 )
												) << paddingLength
										) >> paddingLength;
										j--;
										j--;
